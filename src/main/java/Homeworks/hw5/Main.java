package Homeworks.hw5;

public class Main {

    public static void main(String[] args) {
        Pet pet1 = new Pet("Cat", "Kisa");
        Human motherLennon = new Human("Julia", "Lennon", 1914);
        Human fatherLennon = new Human("Alfred", "Lennon", 1912);
        Human Lennon = new Human("John", "Lennon", 1940, 77);
        Human motherPresley = new Human("Priscilla", "Presley", 1945);
        Human fatherPresley = new Human("Elvis", "Presley", 1935);
        Human Presley = new Human("Lisa Marie", "Presley", 1968);
        System.out.println(Lennon);
        System.out.println(motherLennon);
        System.out.println(fatherLennon);
        System.out.println(Presley);
        System.out.println(motherPresley);
        System.out.println(fatherPresley);
        System.out.println(pet1);

    }
}
